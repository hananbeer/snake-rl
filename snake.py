...
class SnakeGame:
  def __init__(self, width: int, height: int):
    self.width = width
    self.height = height
    self.direction = Vector(1, 0) # +X = right
    self.reset()
  ...
  def reset(self):
    ...
    grid_x = random.randint(2, self.width - 4)
    grid_y = random.randint(2, self.height - 4)
...
class SnakeGameFrontend:
  def __init__(self, game: SnakeGame, block_size: int = 20, play_mode: str = 'absolute', tick_rate: float = 0):
    self.game = game
...
if __name__ == '__main__':
  ...
  frontend = SnakeGameFrontend(game, tick_rate=20)
